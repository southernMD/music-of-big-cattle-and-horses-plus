...
const dowloadAll = async (id) => {
    ...
    songs.forEach((item)=>{
        ...
        globalVar.downloadList.push({ id: item.id, name: cleanFileName, controller: new AbortController(), ifcancel: false, url: '', downloadingFlag: true })
    })
...
const download = async (id: number | string) => {
    ...
    const result = await Main.reqSongDetail([numericId])
    destory()
    let songName = ''
