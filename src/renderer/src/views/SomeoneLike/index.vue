<template>
  <div class="SomeoneLike">
    <div class="title">{{ $route.query.name }}的关注</div>
...
const load = async()=>{
  if(more.value){
    ...
    if(localStorage.getItem('NMcookie')){
      result = await NM.requserFollows(+$route.query.id!,limit.value,offset.value)
    }else{
      result = await Main.requserFollows(+$route.query.id!,limit.value,offset.value)
    }
    more.value = result.more
    offset.value+=limit.value
    let follows = result.follow as Array<any>
