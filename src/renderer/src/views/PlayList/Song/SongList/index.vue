<template>
    ...
    <div v-else class="music-list" @mouseleave="removeBottomColor" :class="{dragMouseStyleCan:Main.dragMouse && Main.dragType == 'songMy',
    'music-list-oneself':globalVar.oneself == 1
    }" id="song-sheet" v-infinite-scroll="load">
        <LineMusic v-if="searchKey.length == 0" v-for="(value, index) in forLength" :index="value" :title="list[index]?.name || ''" :singer="list[index]?.ar || ['']"
            :zhuanji="list[index]?.al || ''" :time="list[index]?.dt  || 0" :id="list[index]?.id || 0" :tns="list[index]?.tns"
            :alia="list[index]?.alia" :key="list[index]?.id"
            :show-index="true"
            :length="forLength"
            @warpPlace="warpPlace"
        ></LineMusic>
        <LineMusic v-else v-for="(value, index) in searchLength" :index="listCopy[index]?.indexList" :title="listCopy[index]?.name || ''"  :singer="listCopy[index]?.ar || ['']" 
            :zhuanji="listCopy[index]?.al || ''" :time="listCopy[index]?.dt || 0" :id="listCopy[index]?.id || 0" :tns="listCopy[index]?.tns"
            :alia="listCopy[index]?.alia" :key="listCopy[index]?.id"
            :show-index="true"
            :indexSearch="value"
            :length="searchLength"
            @warpPlace="warpPlace"
...
import { inject, Ref, watch, ref, shallowRef,toRef } from 'vue'
...
watch(searchKey,()=>{
    if(searchKey.value!=''){
        listCopy.value = [];
        list.value.forEach((element,index,array)=>{
...
watch(routeId, async () => {
    if (route.fullPath.startsWith('/app/playlist/song')) {
        ...
        list.value = (await Main.reqPlaylistTrackAll(id.value)).data.songs
        let arr:[any] | []= []
...
const load = ()=>{
    if(forLength.value + 500 > songNumber.value && songNumber.value - forLength.value > 0) forLength.value +=songNumber.value - forLength.value
    else if(songNumber.value - forLength.value > 0) forLength.value+=500
}
