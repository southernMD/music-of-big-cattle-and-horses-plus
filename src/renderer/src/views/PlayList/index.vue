        ...
        <div class="top">
            ...
            <div class="main" >
                ...
                <div class="author">
                    ...
                    <span class="author-name" :class="{noDrag:!Main.dragMouse,'author-name-oneself':globalVar.oneself == 1}">{{ playList[index]?.creator.nickname }}</span>
                    <span class="createtime" :class="{'createtime-oneself':globalVar.oneself == 1}">{{ dayjsStamp(playList[index]?.createTime) }}创建</span>
                </div>
                <div class="button">
                    <div class="playAll">
                    ...
                    <div class="suo h" v-if="suoFlag" :class="{noDrag:!Main.dragMouse,'h-oneself':globalVar.oneself == 1}">
                ...
                <div class="small">
                    <div class="tags">
                        ...
                        <span class="add" v-if="tags.length === 0 && isMy == 'true' " :class="{noDrag:!Main.dragMouse,'add-oneself':globalVar.oneself == 1}">添加标签</span>
                        <span v-else v-for="(value, index) in tags" :key="value" class="add" :class="{noDrag:!Main.dragMouse}"> 
                            {{ value }}
                    ...
                    <div class="number">
                        ...
                        <span class="num kong" :class="{'num-oneself':globalVar.oneself == 1}">{{ playList[index]?.trackCount }}</span>
                        <span class="title">播放&nbsp;:&nbsp;</span>
                        <span class="num" :class="{'num-oneself':globalVar.oneself == 1}">{{ numberSimp(dynamic?.playCount) }}</span>
                    </div>
                    <div class="describe">
                        <span class="title">简介&nbsp;:&nbsp;</span>
                        <span class="add" v-if="!playList[index]?.description && isMy == 'true' " :class="{noDrag:!Main.dragMouse,'add-oneself':globalVar.oneself == 1}">添加简介</span>
                        <span class="txt" :class="{'txt-oneself':globalVar.oneself}" id="description" v-html="playList[index]?.description">
                        </span>
        ...
        <div class="bottom">
            <div class="choice">
                <div class="tags-three">
                    <Tag class="tag-play" :oneself="1" :class="{'tag-play-oneself':globalVar.oneself == 1}" message="歌曲列表"  :ifClick="flagList[0]" :big="true"  @click="goRoute('songPlaylist');changeTag(0)">
                    </Tag>
                    <Tag class="tag-play" :oneself="1"  :class="{'tag-play-oneself':globalVar.oneself == 1}" :message="`评论(${dynamic?.commentCount})`" :ifClick="flagList[1]" :big="true"  @click="goRoute('commentPlaylist');changeTag(1)">
                    </Tag>
                    <Tag class="tag-play" :oneself="1" :class="{'tag-play-oneself':globalVar.oneself == 1}" message="收藏者"  :ifClick="flagList[2]" :big="true" @click="goRoute('whoStartPlaylist');changeTag(2)"></Tag>
                </div>
                <div class="search">
                    <input type="text" v-model="searchKey" placeholder="搜索歌单内音乐" :class="{noDragInput:!Main.dragMouse,
...
import {
    ...
    toRefs, reactive, getCurrentInstance, ComponentInternalInstance
} from 'vue'
